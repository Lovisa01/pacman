...
public class Main extends Application {
	...
	public void start(Stage gameStage) throws Exception {
		...
		GameFrame frame = new GameFrame(model, SCREEN_WIDTH, SCREEN_HEIGHT - 25);
		Scene gameScene = new Scene(frame);
		// Set the target number of frames per second
		final double targetFps = 75.0;
		 ...
		 * 
		 * @Override public void handle(KeyEvent event) { // We send it on to the model,
		 * to handle it in the various // states of the game. model.keyPressed(event); }
		 * });
		 */
